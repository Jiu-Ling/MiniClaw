...
- **LLM memory summarizer + retrieval overhaul** — BackgroundScheduler + parent-child chunking + LLM rewrite with intent routing + LLM consolidation with critical/normal tiering. Destructive migration via `miniclaw memory rebuild` CLI. Spec: `docs/superpowers/specs/2026-04-15-llm-memory-summarizer-design.md`. Status: Phase 0-4 shipped + cross-cutting refactor done (getattr cleanup, achat_structured adoption, safe-trace consolidation); Phase 5 (migration CLI) + Phase 6 (trace viewer) pending. Goal 7 revised: LoC constraint applies to `miniclaw/memory/` subsystem only (global LoC growth accepted for new capabilities).

---
name: skill-creator
description: Create new SKILL.md files with proper structure, metadata, and scripts. Load this when user asks to create, edit, or package a skill.
---
...
For MiniClaw, custom skills should live under `.miniclaw/skills/` so they can be discovered automatically at runtime (for example, `.miniclaw/skills/my-skill/SKILL.md`).
...
```bash
scripts/init_skill.py my-skill --path .miniclaw/skills
scripts/init_skill.py my-skill --path .miniclaw/skills --resources scripts,references
scripts/init_skill.py my-skill --path .miniclaw/skills --resources scripts --examples
```
...
Keep frontmatter minimal. In MiniClaw, `metadata`, `discoverable`, and `always_active` are also supported when needed, but avoid adding extra fields unless they are actually required.
